...
class read_data():
    ...
    def __init__(self, start_date, end_date, site_no):
        ...
        # get the data file from the website (http://waterdata.usgs.gov)
        URL = urllib.urlopen\
        ...
        # read data using for-loop (except headerline)              
...
if __name__ == "__main__":
    ...
    # plot the results (timeseries of discharge)
    fig, ax = plt.subplots()
    fig.autofmt_xdate()
    ax.plot(plt_dates, plt_flow, 'b', label = "Daily",  lw = 1.5)
    ax.plot(plt_dates, plt_mean, 'k', label = "Annual", lw = 1.5)
    ax.plot(plt_dates, plt_upstd, ':', color = '0.25',  lw = 1.0)
    ax.plot(plt_dates, plt_lwstd, ':', color = '0.25',  lw = 1.0) 
    plt.title('Discharge timeseris from 2010', size = 16)
    plt.xlabel('Dates (mm/yy)', size = 12)
    plt.ylabel('Discharge (m$^{3}$/sec)', size = 12)
    plt.legend()
    #fig.tight_layout()
    plt.show()

    # save the figure
    plt.savefig('HW2_by_Jun.png', dpi=600)